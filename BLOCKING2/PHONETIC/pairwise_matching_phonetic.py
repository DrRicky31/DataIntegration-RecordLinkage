...
data = pd.read_csv('BLOCKING2/PHONETIC/phonetic_merged.csv', low_memory=False)
...
# Creazione di una lista di tutte le coppie per ogni cluster basato sul 'phonetic_name'
all_matches = []

# Loop sui cluster basati sul phonetic_name (senza calcolare nuovamente il phonetic_name)
for cluster_id, group in data.groupby('phonetic_name'):
    ...
    for name1, name2 in combinations(names, 2):
        ...
        all_matches.append({'PhoneticCluster': cluster_id, 'Name1': name1, 'Name2': name2, 'Similarity': similarity})
...
if matches_df.empty:
    ...
    # Salvataggio su CSV
    matches_df.to_csv('BLOCKING2/PHONETIC/phonetic_matching_results.csv', index=False)

    print("Matching completato. Risultati salvati in 'phonetic_matching_results.csv'.")
